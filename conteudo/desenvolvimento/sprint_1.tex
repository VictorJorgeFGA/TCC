...
Na condução dessa análise, o Brasil Participativo foi executado em modo desenvolvimento com nível de \textit{logs} como \textit{debug}. Com esse nível de informações, é possível visualizar na saída padrão do Puma, o servidor \textit{web}, quais consultas SQL estão sendo executadas, quais \textit{views} estão sendo renderizadas pelo \textit{Rails} durante a requisição, e os acessos ao ActiveStorage. Alguns desses registros são acompanhados pelo tempo gasto na execução e em alguns casos a quantidade de alocações também é informada.
...
No repositório oficial do \textit{Ruby on Rails} existe uma \textit{issue} que relata problemas de desempenho com o uso de \textit{partials} em contextos em que há aninhamento, destacando que a renderização de HTML planos acontece com maior agilidade. A \textit{issue} pode ser acessada em \url{https://github.com/rails/rails/issues/41452}.
...
A versão anterior à modificação pode ser vista na Figura \ref{fig:versao-com-celula}, e a versão ajustada para melhoria de desempenho pode ser vista na Figura \ref{fig:versao-inline}.
...
Outro desafio relacionado à inserção do texto através do comando copiar e colar é separar corretamente cada parágrafo de texto. O \textit{input} do usuário, apesar de parecer apenas texto, na verdade é enviado para o \textit{backend} como uma \texttt{string} HTML, como por exemplo:

\textit{
\texttt{<h1>}Título do Texto\texttt{</h1>}
\texttt{<div>}
  Parágrafo! Aqui vem o conteúdo do parágrafo do usuário.\texttt{<br>}
  Aqui é outro parágrafo.\texttt{<br>}
\texttt{</div>}
}
...
\begin{itemize}
	...
	\item Subtítulo (enumerador \texttt{sub\_section}), caso seja \texttt{<h2>} e \texttt{<h3>};
...
Caso o parágrafo seja do tipo imagem, o atributo \texttt{signed\_id} é usado para localizar o arquivo no \texttt{storage} e realizar o vínculo.
